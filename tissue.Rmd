...
Each main tab includes sub-tabs or drop-down menus for selecting and viewing different content. For all analyses, multiple congeners for the same constituent were summed to create the total tissue concentration for DDT and PCB.  Also, multiple samples taken during the same day were averaged in many cases. 
...
powplo1 <- reactive({
  ...
  validate(
    need(nrow(na.omit(powdat)) > 0, 'Insufficient data')
  )
  ...
  p <- ggplot(powdat, aes(x = Date, y = Result)) +
    ...
    labs(
      ...
      subtitle = paste(stasel, typsel, sppsel, sep = ', '),
      xlab = NULL,
      y = paste0(varsel, '')
    )
...
column(12,
  column(6,
    ...
    #   
    # })
    selectInput('typsel', 'Select tissue type:', choices = c("bird egg", "comp", "fillet", "remaining tissue"))
  ),
  column(6,
    renderUI({
      ...
      selectInput('sppsel', 'Select species:', choices = tosel)
