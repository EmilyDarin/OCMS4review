...
optextplo <- reactive({
  ...
  # contours
  p2 <- ggplot(toplo) +
    geom_contour(aes(x = yrs, y = chg, z = pow, group = sta), breaks = as.numeric(powsel), size = 0.5) + 
    # geom_text_repel(data = opts, aes(x = yrs, y = chg, label = sta)) +
    geom_point(data = opts, aes(x = yrs, y = chg, group = sta, size = qntwgt), pch = 21, fill = 'lightgrey', colour = 'black') + 
    scale_size_continuous(guide = F) + 
...
optextplo2 <- reactive({
  ...
  # contours
  p2 <- ggplot(toplo) +
    geom_contour(aes(x = yrs, y = chg, z = pow, group = stapar), breaks = as.numeric(powsel2), size = 0.5) + 
    # geom_text_repel(data = opts, aes(x = yrs, y = chg, label = sta)) +
    geom_point(data = opts, aes(x = yrs, y = chg, group = stapar, size = qntwgt), pch = 21, fill = 'lightgrey', colour = 'black') + 
    scale_x_continuous(limits = c(5, 50)) + 
    scale_size_continuous(guide = F) + 
    thm3 +
    labs(
