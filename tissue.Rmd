...
Each main tab includes sub-tabs or drop-down menus for selecting and viewing different content. For all analyses, multiple congeners for the same constituent were summed to create the total tissue concentration for __DDT__, __PCB__, and __Se__.  Also, multiple samples taken during the same day were averaged in many cases. 
...
mulplo <- reactive({
  ...
  p1 <- ggord(ord, size = 1, alpha = 0.4, vec_ext = 5, coord_fix = F) +
...
opttxt <- reactive({
  ...
  out <- paste0("Optimal sample effort at ", powsel, " power: ", medv , ' relative effort')
...
opttxt2 <- reactive({
  ...
  out <- paste0("Optimal sample effort at ", powsel2, " power: ", medv , ' relative effort')
...
These two maps show the monitoring stations where tissue sampling data are collected.  The top map shows the number of parameters (all congeners) measured at each station and the bottom plot shows the number of years for available data.  The maps are linked so that the mouse pointer is synced between the two.
...
column(12,
  column(6,
    selectInput('varsel', 'Select constituent:', choices = tops)
  ),
  column(6,
    selectInput('stasel', 'Select station:', choices = unique(tsdat$StationCode))
  )
)
```
...
plotlyOutput('optplo2', height = '900px')
```

<br>
<br>
Note: Changing the station and constituent selection above does not affect the estimates since the optimal level is an aggregate across stations and constituents.
...
tsprp <- tsdat %>%
  ...
  filter(Parameter %in% tops) %>%
  select(StationCode, Date, Parameter, Result) %>%
  group_by(StationCode, Date, Parameter) %>%
