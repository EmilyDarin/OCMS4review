...
optwgtmed <- reactive({
  ...
  # weighted median
  out <- weightedMedian(opts$eff, w = opts$qntwgt)
...
opttxt <- reactive({
  ...
  out <- paste0("Optimal sample effort at ", powsel, " power: ", medv , ' relative effort (weighted by distance from threshold: ', wgtmedv, ')')
...
optwgtmedpar2 <- reactive({
  ...
  out <- opts %>% 
    group_by(par) %>% 
    summarise(wgtmed = weightedMedian(eff, w = qntwgt))
...
opttxt2 <- reactive({
  ...
  out <- paste0("Optimal sample effort at ", powsel2, " power: ", medv , ' relative effort (weighted by distance from threshold: ', wgtmedv, ')')
