...
column(12, 
       ...
       column(4, 
              selectInput('varsel2', 'Select constituent:', choices = c(tops, nuts), selected = 'ENT')
              ),
       column(4, 
...
## All Station comparisons {.tabset .tabset-pills}

### Station differences

PCA comparison of all sites, top ten sampled parameters, and complete time series
...
ggord(ord, size = 1, grp_in = wshd, alpha = 0.4, vec_ext = 12, repel = T) + 
...
```{r}
selectInput('dissel', 'Select Parameter:', choices = tops)
```
  ...
  mutate(
    ...
    sitplo = purrr::pmap(list(Parameter, sitcor), function(Parameter, sitcor){
      ...
      p <- ggplot(sitcor) +
        ...
        scale_x_discrete('', expand = c(0, 0)) +
        scale_y_discrete('', expand = c(0, 0)) +
        scale_fill_gradient2('Dissimilarity between stations', low = 'lightblue', mid = 'white', high = 'tomato1', midpoint = 5.5, limits = c(0, 11)) +
        guides(fill = guide_colourbar(barheight = 0.5, barwidth = 10, label.theme = element_text(size = 11, angle = 0))) +
        ggtitle(Parameter) + 
        pbase
...
output$displo <- renderPlot({displo()})
plotOutput('displo', height = "900px", width = "900px")
# combined <- dissim$sitplo[[1]] + dissim$sitplo[[2]] + dissim$sitplo[[3]] + dissim$sitplo[[4]] + dissim$sitplo[[5]] & dissim$sitplo[[6]] + dissim$sitplo[[7]] + dissim$sitplo[[8]] + dissim$sitplo[[9]] + dissim$sitplo[[10]] & theme(legend.position = 'bottom') 
