...
# color palette for hotspot exceedances, all parameters
hotcol2 <- colorNumeric(
  palette = RColorBrewer::brewer.pal(9, 'Reds'),
  na.color = 'yellow',
    domain = c(0, obsaverng[2])
  )
...
hotdat2 <- reactive({
  ...
  out <- dwdat %>% 
    ...
    ungroup() %>% 
    mutate(
      cols = hotcol2(exceeds), 
      cexs = scales::rescale(exceeds, to = c(4, 17), from = obsaverng)
    ) %>% 
    select(Watershed, StationCode, exceeds, n, cols, cexs) %>% 
    unique
...
hotmap2 <- reactive({
  ...
  # add legend
  out <- m@map %>% 
    addLegend("bottomright", pal = hotcol2, title = leglab, opacity = 1, values = tomap$exceeds) 
...
hotplo2 <- reactive({
  ...
  p <- ggplot(toplo, aes(x = StationCode, y = exceeds)) + 
    ...
    scale_y_continuous(limits = c(0, obsaverng[2])) + 
    labs(
      y = ylab
    )
