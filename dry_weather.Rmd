...
# color palette for hotspot exceedances
hotcol <- colorNumeric(
...
# median value for selected constituent
hotmed <- reactive({
...
hotdat <- reactive({
  ...
  out <- dwdat %>% 
    ...
    mutate(
      cols = hotcol(exceeds), 
      cexs = scales::rescale(exceeds, to = c(4, 17), from = obsrng)
    ) %>% 
...
hotplo <- reactive({
  ...
  p <- ggplot(toplo, aes(x = StationCode, y = exceeds)) + 
    ...
    scale_y_continuous(limits = c(0, obsrng[2])) + 
    labs(
      y = ylab
    )
...
column(12, 
       column(4, 
              selectInput('varsel3', 'Select constituent:', choices = c(tops, nuts), selected = 'ENT')
       ),
       column(8, 
...
Map shows number of exceedances of each station above the median value across all dates/stations for `r renderText({input$varsel3})` (`r renderText({round(hotmed(), 1)})`)
```{r}
