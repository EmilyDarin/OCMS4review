...
# OC MS4 monitoring program, harbors and estuaries{.tabset}
...
# sampling location
lxs <- c('Water column' = 'wc', 'Sediment' = 'sd')
...
invtab <- reactive({
  ...
  sums <- sddat %>%
    filter(location %in% locsel1) %>% 
    filter(Parameter %in% tofilt)
  
  tofilt <- tofilt[tofilt %in% unique(sums$Parameter)]
...
powplo3 <- reactive({
  ...
  toplo <- sdpows %>%
    ...
    filter(loc %in% locsel2) %>% 
    filter(typ %in% smpsel)
...
stapardat2 <- reactive({
  ...
  validate(
    need(stasel2 != stasel3, 'Do not select the same station')
  )
  
  out <- sddat %>%
    filter(location %in% locsel3) %>% 
...
obsp4 <- reactive({
  ...
  toplo <- stapardat2
  ttl <- unique(toplo$Parameter)
  ylab <- paste0('Concentration (', unique(toplo$Units), ')')
...
disdat <- reactive({
  ...
  varsel3 <- input$varsel3
  locsel3 <- input$locsel3
  shdsel2 <- input$shdsel2

  req(varsel3)

  if(shdsel2 == 'all')
    shdsel2 <- unique(sdprp$Watershed)

  dissim <- sdprp %>%
    filter(location %in% locsel3) %>% 
    filter(Watershed %in% shdsel2) %>% 
    filter(Parameter %in% varsel3) %>%
    select(-Watershed, -location, -Parameter) %>%
  ...
  validate(
    need(nrow(na.omit(dissim)) > 0, 'Insufficient data')
  )
...
mulplo <- reactive({
  ...
  toord <- sdprp %>%
    filter(Watershed %in% shdsel2) %>% 
    filter(location %in% locsel3) %>% 
    filter(Parameter %in% get(consel3)) %>%
    select(-location) %>% 
...
optdat <- reactive({
  ...
  dat <- sdpows %>%
    filter(par %in% varsel) %>%
    filter(loc %in% locsel2) %>% 
    filter(sta %in% stasels) %>% 
    filter(typ %in% smpsel)

  # get median values of constituent for each station
  # calculate diff from threshold as a weight for the size in the plots
  # if a threshold is available, the weight is inverse of percent difference between median and threshold
  # if a threshold is unavailable, the weight is inverse of percent difference between ecdf of median and 0.5
  meddat <- sddat %>%
    filter(Parameter %in% varsel) %>%
    filter(Watershed %in% shdsel) %>% 
    ...
    filter(location %in% locsel2) %>% 
    filter(Type %in% smpsel) %>% 
    left_join(varthr, by = c('Watershed', 'Parameter', 'location')) %>% 
    group_by(StationCode) %>%
    summarise(
      medval = median(Result, na.rm = T), 
      Threshold = unique(Threshold), 
      .groups = 'drop'
      ) %>%
    mutate(
      wgt = case_when(
        is.na(Threshold) ~ 0.1, #1 / (abs(qntval - 0.5) / 0.5),
        T ~ 1 / (abs(medval - Threshold) / Threshold)
      ), 
      wgt = ifelse(is.infinite(wgt), 1, wgt),
      reltyp = case_when(
        is.na(Threshold) ~ 'none',
        T ~ 'threshold'
      )
    ) %>% 
    rename(
...
opttxt <- reactive({
  ...
  out <- paste0("Optimal sample effort in ", locsel2, " during ", smpsel, "weather at ", powsel, " power: ", medv)
...
optplo <- reactive({
  ...
  # make group for mouseover
  opts <- optdat$opts %>% 
    mutate(
      labs = paste0(sta, ', effort: ', round(eff, 2), ', change: ', round(chg, 2), ', median: ', round(medval, 2), ', threshold: ', round(Threshold, 2), ', weight: ', round(wgt, 2), ', weight type: ', reltyp
                    )
    )
  ...
  p2 <- ggplot(toplo) +
    ...
    geom_point(data = opts, aes(x = eff, y = chg, group = labs, size = wgt), pch = 21, fill = 'lightgrey', colour = 'black') +
    scale_size_continuous(guide = F, limits = c(0, NA)) +
...
# threshold value for constituent
varthr2 <- reactive({
...
optdat2 <- reactive({
  ...
  # if a threshold is unavailable, the weight is inverse of percent difference between ecdf of median and 0.5
  meddat <- sddat %>%
    filter(Parameter %in% get(consel2)) %>%
    filter(Watershed %in% shdsel) %>% 
    filter(StationCode %in% dat$sta) %>%
    filter(location %in% locsel2) %>% 
    filter(Type %in% smpsel) %>% 
    left_join(varthr2, by = c('Watershed', 'Parameter', 'location')) %>% 
  ...
  opts <- try({sdpows %>%
    ...
    dplyr::select(-opt) %>%
    na.omit %>%
    unite('stapar', sta, par, remove = F, sep = ', ') %>%
    ungroup %>% 
    rename(Parameter = par) %>% 
    left_join(meddat,by = c('sta', 'Parameter'))
  })
  ...
  validate(
    need(!inherits(opts, 'try-error'), 'Insufficient data')
  )
  ...
  validate(
    need('eff' %in% names(opts), 'Insufficient data')
  )
...
opttxt2 <- reactive({
  ...
  opts <- optdat2$opts
  medv <- median(opts$eff, na.rm = T) %>% round(2)
  out <- paste0("Optimal sample effort in ", locsel2, " during ", smpsel, " weather at ", powsel2, " power: ", medv)
...
optplo2 <- reactive({
  ...
  # make groups for mouseover
  opts <- optdat2$opts %>% 
    mutate(
      labs = paste0(sta, ', ', Parameter, ', effort: ', round(eff, 2), ', change: ', round(chg, 2), ', median: ', round(medval, 2), ', threshold: ', round(Threshold, 2), ', weight: ', round(wgt, 2), ', weight type: ', reltyp
                    )
    )
...
opttab2 <- reactive({
  ...
  totab <- optdat2$opts %>%
    select(Parameter, sta, eff, medval, Threshold)
...
alltrnd <- reactive({
  ...
  # input
  varsel4 <- input$varsel4
  locsel4 <- input$locsel4
  smpsel2 <- input$smpsel2

  req(varsel4)

  out <- sddat %>%
    filter(Parameter %in% varsel4) %>%
    filter(location %in% locsel4) %>% 
    filter(Type %in% smpsel2) %>% 
    mutate(
      Year = year(Date),
      Month = month(Date, label = T)
    ) %>%
...
alltrndmap <- reactive({
  ...
  out <- mapview(tomap, zcol = 'trend', cex = cexv, label = labs, col.regions = c('tomato1', 'lightgreen'), legend = F, map.types = mptyps)@map
...
output$downloadData <- downloadHandler(
  filename = function() {"harbors_estuaries.csv"},
  content = function(file) {
    write.csv(sddat, file, row.names = FALSE)
  }
)
```

This website presents an evaluation of the Orange County harbors and estuaries monitoring dataset. The content is separated into four main tabs.
...
```{r}
output$invtab <- renderReactable(invtab())
fillCol(reactableOutput('invtab'))
```

## Trends and power analyses {.tabset .tabset-pills}

This tab shows trends and power analysis results for selected constituents. The drop-down menus are filters to select the data to observe. The constituent type filters which constituents can be selected.  The watershed filters the stations that can be selected and the sampling location selection filters water column or sediment sampling for the selected station/constituent.  The sampling weather filter selects data based on dry weather or wet weather sampling (__note that there is no wet weather sampling for sediment data__). Each sub-tab contains additional information about the results.
...
column(12,
  ...
  column(6,
       selectInput('shdsel', 'Select watershed:', choices = c('all', unique(sddat$Watershed)))
      )
)
...
column(12,
  ...
  column(6,
     selectInput('locsel2', 'Select sampling location:', choices = lxs)
  )
)
...
column(12, 
  column(6,
    selectInput('thrsel', 'Filter sites (optimal effort plots only):', choices = c('all', 'Thresholds only'))
  )
)
```
...
These plots show the observed time series for `r renderText(input$varsel)` at `r renderText(input$stasel)` sampled in the `r renderText(tolower(names(lxs[which(unlist(lxs) == input$locsel2)])))`. The top plot is the raw data, the middle plot shows boxplots of the raw data grouped by year, and the bottom plot shows boxplots of the raw data grouped by month.  Points in red in the top plot are those that were at or below the detection limit for the constituent.
...
This sub-tab shows the annual trends for `r renderText(input$varsel)` at `r renderText(input$stasel)` sampled in the `r renderText(tolower(names(lxs[which(unlist(lxs) == input$locsel2)])))`.  The top plot shows trends for the time series after removing the overall mean. This is an effective approach for trend analysis by evaluating deviation of the annual averages relative to the grand mean. Bars in green show annual averages greater than the grand mean and bars in red show annual averages less than the grand mean.  The bars are centered on the zero line, which is a reference for the grand mean. The y-axis reports the grand mean of the constituent in parentheses. The trend is shown as the dotted line for a regression of the annual averages over time.
...
Each grey point (optimal effort) is also sized in proportion to how close the median value of `r renderText({input$varsel})` is to the threshold.  Larger points indicate the median is close to the threshold, suggesting that changes in sample effort should be more carefully examined than stations where the median is farther from the threshold.  Thresholds were obtained from OCPW and are approximate in some cases. In the absence of a threshold, the point is sized smaller relative to the other points (note that sizing is relative to each plot, point sizes cannot be compared across plots).  Mouseover a point to see details (station, effort, change, median value, thresholds if applicable, weighting for point size, and weighting type as either "none" if no threshold or "threshold").
...
sdprp <- sddat %>%
  ...
  select(Watershed, StationCode, Date, Parameter, location, Result) %>%
  group_by(Watershed, StationCode, Date, Parameter, location) %>%
  summarise(Result = mean(Result, na.rm = T), .groups = 'drop') %>%
...
column(12,
  ...
  column(4, 
    selectInput('locsel3', 'Select sampling location:', choices = lxs)     
  ),
  column(4,
    renderUI({
...
column(12,
  column(4,
    selectInput('shdsel2', 'Select watershed:', choices = c('all', unique(sddat$Watershed)))
  ),
  column(4,
    renderUI({
      ...
      if(shdsel2 == 'all')
        shdsel2 <- unique(sddat$Watershed)
      ...
      selectInput('stasel2', 'Select first station:', choices = sels)
  ...
  column(4,
    renderUI({
      ...
      selectInput('stasel3', 'Select second station:', choices = sels, selected = sels[2])
...
column(12,
  column(6,
    selectInput('consel4', 'Select constituent type:', choices = list('Metals' =  'metals', 'Nutrients' = 'nutrs', 'Organic' = 'organs'),  selected = 'Nutrients')
  ),
  column(6, 
    selectInput('locsel4', 'Select sampling location:', choices = lxs)     
  )
)
column(12, 
  column(6,
    renderUI({
      ...
      selectInput('varsel4', 'Select constituent:', choices = sels)

    })
  ), 
  column(6,
     selectInput('smpsel2', 'Select sampling weather:', choices = wx)
  )
)
