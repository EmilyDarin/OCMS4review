...
opttxt <- reactive({
  ...
  out <- paste0("Optimal sample effort in ", locsel2, " at ", powsel, " power: ", medv)
...
optplo <- reactive({
  ...
  opts <- optdat$opts %>% 
    mutate(
      labs = paste0(sta, ', effort: ', round(eff, 2), ', change: ', round(chg, 2), ', median: ', round(medval, 2), ', threshold: ', round(Threshold, 2), ', quantile: ', round(qntval, 2), ', weight: ', round(wgt, 2), ', weight type: ', reltyp
                    )
    )
...
opttxt2 <- reactive({
  ...
  opts <- optdat2$opts
  medv <- median(opts$eff, na.rm = T) %>% round(2)
  out <- paste0("Optimal sample effort in ", locsel2, " at ", powsel2, " power: ", medv)
...
optplo2 <- reactive({
  ...
  opts <- optdat2$opts %>% 
    mutate(
      labs = paste0(sta, ', ', Parameter, ', effort: ', round(eff, 2), ', change: ', round(chg, 2), ', median: ', round(medval, 2), ', threshold: ', round(Threshold, 2), ', quantile: ', round(qntval, 2), ', weight: ', round(wgt, 2), ', weight type: ', reltyp
                    )
    )
...
opttab2 <- reactive({
  ...
  totab <- optdat2$opts %>%
    select(Parameter, sta, eff, medval, qntval, Threshold)
  ...
  out <- reactable(totab,
                   ...
                   columns = list(
                     ...
                     qntval = colDef(name = 'Quantile of median value'), 
...
This tab shows trends and power analysis results for selected constituents. The drop-down menus are filters to select the data to observe. The constituent type filters which constituents can be selected.  The watershed filters the stations that can be selected.  Finally, the sampling location selection filters water column or sediment sampling for the selected station/constituent.  Each sub-tab contains additional information about the results.
...
Each grey point (optimal effort) is also sized in proportion to how close the median value of `r renderText({input$varsel})` is to the threshold.  Larger points indicate the median is close to the threshold, suggesting that changes in sample effort should be more carefully examined than stations where the median is farther from the threshold.  Thresholds were obtained from OCPW and are approximate in some cases. In the absence of a threshold, the point is sized relative to the overall median for a parameter across all sites.  Mouseover a point to see details (station, effort, change, median value, thresholds if applicable, quantile of median relative to other sites, weighting for point size, and weighting type as either quantile if relative to median or threshold if relative to threshold).
