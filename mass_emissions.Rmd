...
PCA comparison of all stations, nutrients, and complete time series
...
ggord(ord, size = 1, grp_in = wshd, alpha = 0.4, vec_ext = 5) + 
...
These plots show measures of dissimilarity for each parameter between the time series at each station.  Blue tiles are more similar and red are more dissimilar.  
...
dissim <- meprp %>% 
  ...
  nest %>% 
  mutate(
    sitcor = purrr::map(data, function(x){
    ...
    sitplo = purrr::pmap(list(Parameter, sitcor), function(Parameter, sitcor){
...
combined <- dissim$sitplo[[1]] + dissim$sitplo[[2]] + dissim$sitplo[[3]] + dissim$sitplo[[4]] + dissim$sitplo[[5]] & theme(legend.position = 'bottom') 
...
```{r}
renderPlot({obsp4()}, height = 550, width = 700)
renderPlot({obsp5()}, height = 550, width = 700)
renderPlot({obsp6()}, height = 550, width = 700)
```
