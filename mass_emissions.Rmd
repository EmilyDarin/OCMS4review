...
browser()
  ...
  meddat <- medat %>% 
    ...
    mutate(
      ...
      qntdif = case_when(
        length(varthr) != 0 ~ abs(medval - varthr),
        T ~ abs(qntval - 0.5)
      ),
      qntwgt = case_when(
        length(varthr) == 0 ~ 1 - qntdif, 
        length(varthr) != 0 ~ 1 / qntdif
      )
    ) %>% 
...
optplo <- reactive({
  ...
  opts <- optdat$opts %>% 
    mutate(
      labs = paste0(sta, ', effort: ', round(eff, 2), ', change: ', chg, ', median concentration: ', medval, ', threshold: ', varthr)
    )
  ...
  p2 <- ggplot(toplo) +
    ...
    geom_point(data = opts, aes(x = eff, y = chg, group = labs, size = qntwgt), pch = 21, fill = 'lightgrey', colour = 'black') + 
