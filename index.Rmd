  ...
  mutate(
    ...
    sigs = purrr::map(simdat, function(x){
      ...
      x <- x %>% 
        ...
        nest %>% 
        mutate(
          pval = purrr::map(data, ~kendallSeasonalTrendTest(exp(simrand) ~ Month + Year, data = .x)$p.value[2]),
          sig = case_when(
            pval < 0.05 ~ 'sig', 
            pval >= 0.05 ~ 'nonsig'
          )
...
p <- ggplot(toplo, aes(x = Date, y = exp(simrand), group = sims, col = sig)) + 
  ...
  labs(
    title = 'Observed nitrogen (mg/L)', 
    subtitle = 'SDMF05, ten simulations per scenario', 
...
toplo <- scns %>% 
  mutate(
    pow = purrr::pmap(list(simdat, thr), function(simdat, thr) thrfun(simdat, thr))
  ) %>% 
  unnest(c(simdat, pow)) %>% 
  ungroup %>% 
  mutate(
    ...
    pow = paste('power', pow), 
    abv = case_when(
      abv ~ 'Detected in time series', 
      !abv ~ 'Not detected in time series'
    )
  ) %>% 
  unite('grp', thr, eff, pow, sep = ', ', remove = F) %>% 
  mutate(
...
p <- ggplot(toplo, aes(x = Date, y = exp(simrand), group = sims, col = abv)) + 
  geom_line(alpha = 0.2) + 
  geom_point(size = 0.5, aes(shape = val)) +
...
This plot shows a summary of power for several sampling efforts and exceedance thresholds. Power is based on 10000 simulations for each scenario (differing from the example above). Power is lower for higher exceedance values and less frequent sampling, wheras power increases with lower exceedance values and higher sampling. 

```{r fig.height = 5, fig.width = 6, out.width = '70%', fig.align = 'center'}
p <- ggplot(thrs, aes(x = eff, y = pow, colour = thr, group = thr)) + 
  geom_line() + 
  geom_point() + 
  scale_colour_viridis_c('Exceedance value to detect (mg/L)') +#, colors = RColorBrewer::brewer.pal(9, 'Reds')[4:9]) +
  thm + 
  labs(
    y = 'Power', 
    title = 'Power estimates for SDMF05', 
    subtitle = 'Lines show power for detecting at least one obs. > exceedance', 
    x = 'Relative sample effort'
  )
p
```
