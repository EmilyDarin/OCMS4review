...
toplo <- scns %>% 
  mutate(
    ...
    pow = purrr::map(yrspow, function(x) mean(x$yrpow))
  ) %>% 
  unnest(pow) %>% 
  unnest(yrspow) %>% 
  ungroup %>% 
  mutate(
    thr = paste('exceedance', thr),
    eff = paste('effort', eff), 
    pow = paste('avg. power', round(pow, 2))
  ) %>% 
  unite('grp', thr, eff, pow, sep = ', ', remove = F) 

p <- ggplot(toplo, aes(x = factor(Year), y = yrpow)) + 
  geom_bar(stat = 'identity') +
  facet_wrap(~ grp, nrow = 2) +
  thm + 
  labs(
    title = 'Power by year of exceeding threshold', 
    subtitle = 'SDMF05, twenty simulations per scenario', 
    y = 'Power',
    x = NULL
  )
