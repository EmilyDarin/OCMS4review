...
p <- ggplot(toplo, aes(x = Date, y = simrand, group = sims, col = sig)) + 
  geom_line(alpha = 0.2) + 
  geom_point(size = 0.2) +
...
This plot shows a summary of power for several efforts and trends. Power is based on 1000 simulations for each scenario (differing from th example above). Power is low for very small trends regardless of sample effort, whereas power is generally high for larger trends until effort falls below 40-50% of actual. 

```{r fig.height = 5, fig.width = 6, out.width = '80%', fig.align = 'center'}
p <- ggplot(pows, aes(x = eff, y = pow, colour = chg, group = chg)) + 
  geom_line() + 
  geom_point() + 
  scale_colour_gradientn('Magnitude of trend change', colors = RColorBrewer::brewer.pal(9, 'Reds')[4:9]) +
  thm + 
  labs(
    y = 'Power', 
    title = 'Power estimates for SDMF05', 
    subtitle = 'Trend lines show power for varying sample effort and trend magnitude', 
